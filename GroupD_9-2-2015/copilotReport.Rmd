...
copilotTTC <- 35 # insert the co-pilot's estimated time to complete (in minutes, it is fine to approximate) e.g., 120
pilotStartDate <- as.Date("11/03/2019") # insert the piloting start date in US format e.g., as.Date("01/25/18", format = "%m/%d/%y")
copilotStartDate <- as.Date("11/08/2019") # insert the co-piloting start date in US format e.g., as.Date("01/25/18", format = "%m/%d/%y")
...
chi_color_phi <- sqrt(6.4004/40)
 ...
 #I think the psych package has a way to do this but I futzed around a little bit and I think it would actually require more lines of code, so doesn't seem terribly efficient. --em 
